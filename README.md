...
- **Multi-client resize filtering** — when multiple dashboards (desktop, mobile, Playwright) view the same session, only the client that most recently sent user input can resize the tmux pane. This prevents resize wars between clients with different terminal sizes. The force fit button (⊞) always bypasses this check. Ownership expires after 10 seconds of inactivity. If an attached tmux client constrains the window size, the agent automatically sets `window-size manual` on the tmux session and retries the resize. Terminal dimensions are also re-sent when the WebSocket reconnects to handle the race condition where the initial resize fires before the connection is open.
